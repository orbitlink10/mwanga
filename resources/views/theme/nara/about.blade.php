...
@php
    $siteName = (string) get_option('site_name', 'Nara Luxury International');
    $aboutRaw = (string) get_option('about', '');
    $aboutFallback = 'Welcome to our website. Learn more about our services and company.';
    $aboutText = trim(strip_tags($aboutRaw));
    $aboutText = $aboutText !== '' ? $aboutText : $aboutFallback;
    $aboutTextForMeta = trim(preg_replace('/\s+/', ' ', $aboutText));
    $metaDescription = Str::limit($aboutTextForMeta, 160);
    if ($metaDescription === '') {
        $metaDescription = 'About ' . $siteName;
    }

    $pageUrl = url()->current();
    $ogImage = trim((string) get_option('hero_image'));
    if ($ogImage === '') {
        $ogImage = trim((string) get_option('logo'));
    }
    if ($ogImage === '') {
    ...
    $schema = [
        ...
        '@graph' => [
            [
                ...
                'url' => url('/'),
                'logo' => $ogImage,
                'telephone' => preg_replace('/\s+/', '', (string) get_option('contact_phone')),
                'address' => [
                    '@type' => 'PostalAddress',
                    'streetAddress' => (string) get_option('address'),
                    'addressCountry' => 'KE',
                ],
            ],
            [
...
@endphp
@section('title') About {{ $siteName }} @endsection
@section('meta_description', e($metaDescription))
@section('og_title', e('About ' . $siteName))
@section('og_description', e($metaDescription))
@section('og_image', e($ogImage))
@section('og_url', e($pageUrl))
@section('twitter_title', e('About ' . $siteName))
@section('twitter_description', e($metaDescription))
@section('twitter_image', e($ogImage))

@push('meta')
<script type="application/ld+json">
@json($schema, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE)
</script>
@endpush
        ...
        <div class="container">
            <div class="row align-items-end gy-4">
                <div class="col-lg-8">
                    ...
                    <h1 class="about-hero-title about-reveal" style="--delay: .12s;">About {{ $siteName }}</h1>
                ...
                <div class="col-lg-8">
                    <div class="card border-0 wow fadeIn animated about-card about-story">
                        <div class="card-body p-4 p-lg-5">
                            ...
                            <div class="about-copy">{!! nl2br(e($aboutText)) !!}</div>
                        </div>
                    </div>
