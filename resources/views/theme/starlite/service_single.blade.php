...
@php
    $serviceMetaDescription = \Illuminate\Support\Str::limit(
        strip_tags((string) ($service->meta_description ?: $service->description ?: $service->name)),
        160,
        ''
    );
@endphp
        ...
        <div class="container">
            <div class="text-center mb-5">
                ...
                @if($serviceMetaDescription)
                    <p class="lead text-muted">{{ $serviceMetaDescription }}</p>
                @endif
            </div>

            <div class="service-description">
                {!! trim((string) ($serviceDescriptionHtml ?? '')) !== '' ? $serviceDescriptionHtml : $service->description !!}
            </div>
        </div>
    </section>
