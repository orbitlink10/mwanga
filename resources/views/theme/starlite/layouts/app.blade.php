...
<head>
    ...
    @php
        $siteName = get_option('site_name', 'Starlite Internet Kenya');
        ...
        $blockedDomains = [
            ...
            'starlinkugandainstallers.co.ke',
            'starlinknairobiinstallers.co.ke',
            'starlinkkenyadistributors.co.ke',
            'starlinkkenyaresellers.co.ke',
        ...
        $logo = $resolveUrl(get_option('logo'));
        $heroImage = $resolveUrl(get_option('hero_image')) ?: $logo;
        $heroHost = strtolower((string) parse_url((string) $heroImage, PHP_URL_HOST));
        if ($heroHost !== '' && in_array($heroHost, $blockedDomains, true)) {
            $heroImage = $logo;
        }
        if ($heroHost !== '' && $siteHost !== '' && $heroHost !== $siteHost) {
            $heroImage = $logo;
        }
        $contactEmail = trim((string) get_option('contact_email'));
        if ($contactEmail !== '' && filter_var($contactEmail, FILTER_VALIDATE_EMAIL)) {
            $emailDomain = strtolower(substr(strrchr($contactEmail, "@") ?: '', 1));
        ...
        $sameAs = array_values(array_filter([
            ...
            get_option('linkedin'),
        ], static function ($url) use ($blockedDomains): bool {
            $url = trim((string) $url);
            if ($url === '' || !filter_var($url, FILTER_VALIDATE_URL)) {
                return false;
            }
            $host = strtolower((string) parse_url($url, PHP_URL_HOST));
            return $host === '' || !in_array($host, $blockedDomains, true);
        }));
    ...
    <meta property="og:description" content="@yield('og_description', 'Order genuine Starlink kits and book expert installation anywhere in Kenya with Starlite Internet Kenya.')" />
    <meta property="og:image" content="@yield('og_image', $heroImage)" />
    ...
    <meta name="twitter:description" content="@yield('twitter_description', 'Fast, reliable Starlink internet with professional installers across Kenya.')" />
    <meta name="twitter:image" content="@yield('twitter_image', $heroImage)" />
                            ...
                            <div class="d-flex justify-content-end align-items-center py-3">
                                ...
                                <div class="me-3">
                                    <a href="{{ route('cart.view') }}" class="text-dark text-decoration-none d-flex align-items-center">
                                        ...
                                        <span class="badge bg-dark ms-2">{{ $cartCount }}</span>
                                    </a>
                                </div>
