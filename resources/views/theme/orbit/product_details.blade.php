...
@push('meta')
@php
    $productImageUrl = uploaded_asset_url($product->photo, 'default-image.jpg');
@endphp
    ...
    if ($mediafiles->count() > 0) {
        foreach ($mediafiles as $media) {
            if (!empty($media->file_path)) {
                $schemaImages[] = uploaded_asset_url($media->file_path);
            }
        }
                                ...
                                @foreach($mediafiles as $index => $media)
                                    ...
                                    <div class="carousel-item {{ $index == 0 ? 'active' : '' }}">
                                        <div class="ratio ratio-1x1 rounded overflow-hidden">
                                            <img src="{{ $mediaImageUrl }}" alt="{{ $product->name }}" class="w-100 h-100 object-fit-cover" data-bs-toggle="modal" data-bs-target="#imageModal" onclick="openModal('{{ $mediaImageUrl }}', '{{ $index }}')">
                                        </div>
                                    </div>
                                @endforeach
                            @else
                                <div class="carousel-item active">
                                    <div class="ratio ratio-1x1 rounded overflow-hidden">
                                        <img src="{{ $productImageUrl }}" alt="{{ $product->name }}" class="w-100 h-100 object-fit-cover" data-bs-toggle="modal" data-bs-target="#imageModal" onclick="openModal('{{ $productImageUrl }}', '0')">
                                    </div>
                                </div>
                            @endif
                        </div>
                            ...
                            @if($mediafiles->count() > 0)
                                @foreach($mediafiles as $index => $media)
                                    ...
                                    <div class="thumb-item">
                                        <img src="{{ $mediaImageUrl }}" alt="thumb-{{ $index }}" class="img-thumbnail rounded js-thumb {{ $index==0 ? 'border-primary' : '' }}" data-index="{{ $index }}">
                                    </div>
                                @endforeach
                            @else
                                <div class="thumb-item">
                                        ...
                                        @foreach($mediafiles as $index => $media)
                                            ...
                                            <div class="carousel-item {{ $index == 0 ? 'active' : '' }}">
                                                <div class="ratio ratio-4x3">
                                                    <img src="{{ $mediaImageUrl }}" class="w-100 h-100 object-fit-cover">
                                                </div>
                                            </div>
                                        @endforeach
                                    @else
                                        <div class="carousel-item active">
                                            <div class="ratio ratio-4x3">
                                                <img src="{{ $productImageUrl }}" class="w-100 h-100 object-fit-cover">
                                            </div>
                                        </div>
                                    @endif
                                </div>
                                ...
                                <div id="modalThumbsTrack" class="d-flex gap-2 flex-nowrap thumbs-scroll px-4 justify-content-center">
                                    @if($mediafiles->count() > 0)
                                        @foreach($mediafiles as $index => $media)
                                            @php($mediaImageUrl = uploaded_asset_url($media->file_path))
                                            <div class="thumb-item">
                                                <img src="{{ $mediaImageUrl }}" alt="mthumb-{{ $index }}" class="img-thumbnail rounded js-mthumb {{ $index==0 ? 'border-primary' : '' }}" data-index="{{ $index }}">
                                            </div>
                                        @endforeach
                                    @else
                                        <div class="thumb-item">
