...
Route::get('/robots.txt', function () {
    ...
    $fallbackHost = $theme === 'starlite'
        ? 'starliteinternetkenya.co.ke'
        : 'refurbishedphonesinkenya.co.ke';
    $requestedHost = strtolower((string) request()->getHost());
    $canonicalHost = in_array($requestedHost, ['localhost', '127.0.0.1'], true)
        ? $fallbackHost
        : $requestedHost;
...
Route::get('/starlink-kenya-price', function () {
    if ((string) get_option('theme') !== 'starlite') {
        return redirect('/refurbished-phones-prices-kenya', 301);
    }

    return view('theme.'.get_option('theme').'.pricing');
})->name('refurbished.pricing');
Route::get('/refurbished-phones-prices-kenya', function () {
    if ((string) get_option('theme') === 'starlite') {
        return redirect('/starlink-kenya-price', 301);
    }
...
Route::get('/starlink-rental', function () {
    return view('theme.'.get_option('theme').'.starlink-rental');
});
Route::get('/phone-rental-kenya', fn () => redirect('/starlink-rental', 301));
...
Route::get('/refurbished-phones-in-kenya', function (Request $request) {
    if ((string) get_option('theme') === 'starlite') {
        return redirect('/shop', 301);
    }

    return app(WelcomeController::class)->products($request);
